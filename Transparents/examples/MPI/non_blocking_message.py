...
if rank == 0:
    ...
    req = globCom.isend(une_liste, dest=1, tag=101)
    liste_recue = globCom.recv(source=1, tag=102)
    req.wait()
elif rank==1:
    une_liste = [ "Alice", 131, ("Bob", 5.2), [2,3,5,7,11]]
    req = globCom.isend(une_liste, dest=0, tag=102)
